  ...
  <div class="portal-app">
    ...
    <main>
      ...
      <section v-else-if="page === 1" class="settings-page" aria-label="偏好设置">
        ...
        <details class="advanced-settings"><summary>高级连接设置</summary><MiuixCard class="advanced-fields"><label class="field">认证服务器<input v-model="draft.server" :disabled="settingsLocked" spellcheck="false"></label><label class="field">认证网站<input v-model="draft.auth_url" :disabled="settingsLocked" spellcheck="false"></label><label class="field">Portal URL（可选）<input v-model="portalUrl" :disabled="locked" autocomplete="off" spellcheck="false" placeholder="留空自动探测，或粘贴当前网络的认证网址"></label><label class="field">HTTP 探测地址<input v-model="draft.probe_url" :disabled="settingsLocked" spellcheck="false"></label><p class="advanced-note">认证请求会把 paip 固定为 172.18.100.65；wlanuserip、clientip 和 clientmac 使用上面所选网卡的当前值。</p></MiuixCard></details>
