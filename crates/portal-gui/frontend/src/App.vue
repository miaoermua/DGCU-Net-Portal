  ...
  <div class="portal-app">
    ...
    <main>
      ...
      <section v-else-if="page === 1" class="settings-page" aria-label="偏好设置">
        ...
        <details class="advanced-settings"><summary>高级连接设置</summary><MiuixCard class="advanced-fields"><label class="field">认证服务器<input v-model="draft.server" :disabled="settingsLocked" spellcheck="false"></label><label class="field">认证网站<input v-model="draft.auth_url" :disabled="settingsLocked" spellcheck="false"></label><label class="field">Portal URL（可选）<input v-model="portalUrl" :disabled="locked" autocomplete="off" spellcheck="false" placeholder="留空使用 DGCU 模板，或粘贴当前网络的认证网址"></label><label class="field">HTTP 探测地址<input v-model="draft.probe_url" :disabled="settingsLocked" spellcheck="false"></label><p class="advanced-note">默认先使用 DGCU Portal 模板；模板失败且“认证失败后自动探测”开启时，才访问这里的探测地址。认证请求会把 paip 固定为 172.18.100.65。</p></MiuixCard></details>
