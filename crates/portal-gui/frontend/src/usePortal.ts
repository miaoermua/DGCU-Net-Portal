...
export const defaultSettings = (): Settings => ({ server: 'http://172.18.100.65/lfradius/', auth_url: 'http://172.18.100.65/lfradius/web/admin/login', probe_url: 'http://captive.apple.com/hotspot-detect.html', paip: '172.18.100.65', basip: '', probe_enabled: true, refresh_policy: 'one_minute', poll_jitter: 'low', traffic_enabled: false, credential_store: 'system', interface_name: '', bypass_proxy: true, username: '', auto_redial: false, tray_startup: false, service_enabled: false, show_sessions: false, log_enabled: false, theme_mode: 'system' })
...
export function createPortalState(bridge?: DesktopBridge) {
  ...
  const hasUnsavedConnectionSettings = computed(() => {
    const keys: (keyof Settings)[] = ['server', 'auth_url', 'probe_url', 'paip', 'basip', 'probe_enabled', 'refresh_policy', 'poll_jitter', 'traffic_enabled', 'credential_store', 'interface_name', 'bypass_proxy', 'auto_redial', 'tray_startup', 'service_enabled']
