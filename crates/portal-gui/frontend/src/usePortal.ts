...
export function createPortalState(bridge?: DesktopBridge) {
  ...
  const hasUnsavedConnectionSettings = computed(() => {
    const keys: (keyof Settings)[] = ['server', 'auth_url', 'probe_url', 'interface_name', 'bypass_proxy', 'one_session', 'remember_account', 'auto_redial', 'tray_startup', 'service_enabled']
